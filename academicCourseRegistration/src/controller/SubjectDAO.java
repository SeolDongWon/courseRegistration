...
public class SubjectDAO { // Subject Data Access Object
	...
	public void getSubjectTotalList() {
		...
		SubjectVO sVo = null; // 출력결과 데이터로 만든 객체를 저장할 변수

		try {
			String sql = String.format("select * from subject order by no"); // 쿼리를 문자열로 저장
			...
			while (rs.next()) { // 출력결과를 한 줄씩 읽어서 null까지 반복
				sVo = new SubjectVO(); // 출력결과를 받을 객체 할당
				sVo.setNo(rs.getInt("no")); // 출력결과 중 no를 할당
				sVo.setS_num(rs.getString("s_num")); // 출력결과 중 s_num를 할당
				sVo.setS_name(rs.getString("s_name")); // 출력결과 중 s_name를 할당
				System.out.println(sVo.getNo() + "\t" + sVo.getS_num() + "\t" + sVo.getS_name()); // 객체에 할당한 데이터를 출력
			}
	...
	// 학과 등록
	public void setSubjectRegiste(SubjectVO svo) {
		Connection con = DBUtil.getConnection();
		PreparedStatement pstmt = null;

		try {
			String sql = String.format("insert into subject(no, s_num, s_name) values (subject_no_seq.nextval, ?, ?)");
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, svo.getS_num()); // 쿼리 문자열의 1번째 ?에 넣을 변수
			pstmt.setString(2, svo.getS_name()); // 쿼리 문자열의 2번째 ?에 넣을 변수
	...
	public void setSubjectUpdate(SubjectVO svo) {
		...
		try {
			String sql = String.format("update subject set s_num=?, s_name=? where no=?");
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, svo.getS_num()); // subjectManager.subjectUpdate()에서 입력한 데이터 출력
			pstmt.setString(2, svo.getS_name()); // subjectManager.subjectUpdate()에서 입력한 데이터 출력
			pstmt.setInt(3, svo.getNo()); // subjectManager.subjectUpdate()에서 입력한 데이터 출력
	...
	public void setSubjectDelete(int s_no) { // subjectManager.subjectDelete()에서 입력한 데이터를 매개변수로 받음
		...
		try {
			String sql = String.format("delete from subject where no = ?");
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, s_no);
